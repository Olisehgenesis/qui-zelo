...
const useContractTransaction = () => {
  ...
  const executeTransaction = useCallback(async ({
    ...
    try {
      ...
      // Try to get wallet client from connector if hook is not available
      let currentWalletClient = walletClient;
      if (!currentWalletClient && connector) {
        console.log('🔄 Getting wallet client from connector...');
        try {
          currentWalletClient = await (connector as any).getWalletClient({ chainId: targetChainId });
        } catch (error) {
          console.log('Failed to get wallet client from connector:', error);
        }
      }
      ...
      if (!currentWalletClient) {
        ...
        if (!walletClient) {
          throw new Error('Wallet client not available. Please ensure your wallet is properly connected and try again.');
        }
  ...
  }, [sendTransactionAsync, walletClient, publicClient, currentChainId, switchChain, isConnected, address, connector]);
...
export const useQuizelo = () => {
  ...
  const startQuiz = async () => {
    ...
    if (!address || !isConnected) {
      console.log('❌ Wallet not connected in startQuiz:', { address, isConnected });
      showError('Please connect your wallet first');
      return { success: false, sessionId: null };
    }

    console.log('✅ Wallet connected in startQuiz:', { address, isConnected, currentChainId });

    // Check if quiz fee is loaded
    if (!quizFee) {
      showError('Quiz fee not loaded yet. Please wait a moment and try again.');
      return { success: false, sessionId: null };
    }
    ...
    if (currentChainId !== targetChainId) {
      try {
        console.log('🔄 Switching to correct network before starting quiz...');
        await switchChain({ chainId: targetChainId });
        await new Promise(resolve => setTimeout(resolve, 1000));
      } catch (error) {
    ...
    try {
      ...
      console.log('🚀 Executing startQuiz transaction...');
      ...
      console.log('📝 startQuiz transaction result:', result);
  ...
  const claimReward = async (sessionId: string, score: number) => {
    ...
    if (currentChainId !== targetChainId) {
      try {
        console.log('🔄 Switching to correct network before claiming reward...');
        await switchChain({ chainId: targetChainId });
        await new Promise(resolve => setTimeout(resolve, 1000));
      } catch (error) {
        console.error('Failed to switch network:', error);
